 ...
 *
 * 职责二:两个路由。鉴权默认关闭(配置 token 时才要求 X-Dsh-Md-Picker-Token,
 * 可选开关,便于 LAN 加固;未配置 token 时完全开放):
...
export function apply(ctx, rawConfig = {}) {
  ...
  // 鉴权默认关闭:只有显式配置 token 时才校验 X-Dsh-Md-Picker-Token。
  // 未配置时客户端也不会携带该头,请求完全开放(速率限制仍在)。
  const token = config.token || ''
  const targetOverride = config.targetDir || ''

  /** 路由公共护栏:可选 token 鉴权(仅配置了 token 时校验)+ 每路由速率限制 */
  function guard(req, res, routeKey, ratePerMin) {
    if (token && req.headers['x-dsh-md-picker-token'] !== token) {
