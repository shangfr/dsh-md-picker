...
;(function () {
	 ...
	 *  Supports BOTH the legacy <textarea> composer and the current DSH web
	 *  Lexical contenteditable composer ([data-composer-input]). For Lexical,
	 *  execCommand('insertText') fires a real beforeinput the composer's own
	 *  input pipeline consumes, so editor state + draft mirror stay in sync.
	 *  Returns true when the text actually landed. */
	function insertIntoDraft(text) {
		...
		// Current DSH web shell: Lexical contenteditable div, no textarea at all.
		var input = card ? card.querySelector("[data-composer-input]") : document.querySelector("[data-composer-input]");
		if (!input) return false;
		try {
			input.focus();
			var sel = window.getSelection(), range = document.createRange();
			range.selectNodeContents(input);
			range.collapse(false); // caret to the end of the existing draft
			sel.removeAllRanges();
			sel.addRange(range);
			var separator = input.textContent && input.textContent.trim() ? "\n\n" : "";
			var ok = document.execCommand("insertText", false, separator + text);
			return ok !== false;
		} catch (err) {
			console.warn("[dsh-md-picker] contenteditable insert failed:", err);
			return false;
		}
	}

	/** Transient visible notice (fallback when the receipt cannot be placed in
	 *  the draft — guarantees the user always SEES result/error text). */
	function showNotice(text, kind) {
		var host = document.querySelector("[data-composer-card]") || document.body;
		var el = document.createElement("div");
		el.setAttribute("data-plugin", "dsh-md-picker");
		el.setAttribute("data-plugin-notice", "dsh-md-picker");
		el.style.cssText = "position:fixed;right:16px;bottom:96px;z-index:2147483000;max-width:min(480px,calc(100vw - 32px));padding:10px 14px;border-radius:10px;font:12px/1.6 system-ui,sans-serif;white-space:pre-wrap;box-shadow:0 4px 16px rgba(0,0,0,.25);"
			+ (kind === "error" ? "background:#7f1d1d;color:#fee2e2;border:1px solid #ef4444;" : "background:#052e16;color:#dcfce7;border:1px solid #22c55e;");
		el.textContent = text;
		host.appendChild(el);
	...
	function handlePicked(files, btn) {
		...
		Promise.all(jobs).then(function (items) {
			...
			var receipt = blocks.join("\n\n");
			if (!insertIntoDraft(receipt)) {
				// Composer is neither a textarea nor a Lexical host — never drop
				// the outcome silently; show it so the user can read the reason.
				console.warn("[dsh-md-picker] composer not writable, showing notice;", receipt.slice(0, 220));
				showNotice(receipt, convertFailed ? "error" : "success");
			}
		})["catch"](function (e) {
			console.error("[dsh-md-picker]", e);
			showNotice("[dsh-md-picker] 上传/转换流程异常: " + (e && e.message ? e.message : e), "error");
			setState(btn, "error", 1500);
		});
	}
